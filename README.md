...
6競技8チームの日程を各公式サイト/リーグ公式サイトから毎日自動取得している。
...
未対応: バスケットボール(横浜ビー・コルセアーズ、川崎ブレイブサンダース、富士通レッドウェーブ)は
公式サイトが直近数試合しか掲載していないため、月別ページを巡回する実装が必要。
ドッジボールはプロリーグが存在しない。詳細は [docs/DATA_SOURCES.md](docs/DATA_SOURCES.md)。
...
1. バスケットボール3チームの取得(月別ページの巡回が必要)
2. 川崎フロンターレの取得(月別ページのため同上)
3. GitHub Pagesの有効化(リポジトリ設定から`site/`を公開する)
4. 各サイトの利用規約の確認。robots.txtは機械的に確認済みだが、規約本文は未確認
